...
class GroundControlSpec extends FunSpec with GivenWhenThen with ShouldMatchers {
  ...
  describe("A ground control in taxiway management") {
    ...
    it("should tell the plane to taxi when 1 taxiway is free") {
      ...
      val groundControl = system.actorOf(Props(classOf[GroundControl], Seq(taxiway.ref), Seq.empty[ActorRef], 1), "groundControl")
    ...
    it("should fill taxiway until it's full") {
      ...
      implicit val system = ActorSystem()
      val taxiway = TestProbe()
      val groundControl = system.actorOf(Props(classOf[GroundControl], Seq(taxiway.ref), Seq.empty[ActorRef], 4), "groundControl")
      ...
      planes.foreach(_.expectMsg(100 milliseconds, Taxi(taxiway.ref)))
    ...
    it("should allocate free taxiways to each plane") {
      ...
      val groundControl = system.actorOf(Props(classOf[GroundControl], taxiways, Seq.empty[ActorRef], 1), "groundControl")
      ...
      Then("ground control should tell each plane to taxi on the taxiway")
      val replies = planes.map(_.expectMsgAllClassOf(100 milliseconds, classOf[Taxi]).head)
  ...
  describe("A ground control in gate management") {
    ...
    it("should tell the plane to park when there is a free gate") {
      ...
      implicit val system = ActorSystem()
      val gate = TestProbe()
      val groundControl = system.actorOf(Props(classOf[GroundControl], Seq.empty[ActorRef], Seq(gate.ref), 0), "groundControl")
    ...
    it("should alternate the runway allocated to a new incoming plane") {
      ...
      val gate1 = TestProbe()
      val gate2 = TestProbe()
      val groundControl = system.actorOf(Props(classOf[GroundControl], Seq.empty[ActorRef], Seq(gate1.ref, gate2.ref), 0), "groundControl")
      ...
      val allocation1 = plane1.expectMsgAllClassOf(classOf[ParkAt]).head
      val allocation2 = plane2.expectMsgAllClassOf(classOf[ParkAt]).head
    ...
    it("should not tell the plane to park until a gate is free") {
      ...
      val gate = TestProbe()
      val taxiway = TestProbe()
      val groundControl = system.actorOf(Props(classOf[GroundControl], Seq(taxiway.ref), Seq(gate.ref), 1), "groundControl")

      Given("a first plane has parked at the gate")
      val firstPlane = TestProbe()
      firstPlane.send(groundControl, Incoming)
      firstPlane.expectMsg(100 milliseconds, Taxi(taxiway.ref))
      firstPlane reply EndOfTaxi
      firstPlane.expectMsg(100 milliseconds, ParkAt(gate.ref))
      firstPlane reply HasParked
